import { type Game, type PlayerColor } from "@prisma/client";
...
const Profile: NextPage = () => {
  ...
  const { isSuccess: isGamesSuccess } =
    api.socials.getRecentGames.useInfiniteQuery(
      ...
      },
      {
        ...
        onSuccess: (data) => {
          const [currentPageGames] = data.pages;

          if (!currentPageGames) {
            return;
          }

          setGames((x) => x.concat(currentPageGames.games));
        },
        enabled: !!sessionData,
        refetchOnWindowFocus: false,
        refetchOnMount: false,
  ...
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-neutral-900">
      ...
      <div className="flex flex-col gap-1 px-5 py-3">
        ...
        {isGamesSuccess &&
          games.map((game) => {
            if (!game.gameToUsers[0] || !game.gameToUsers[1] || !sessionData) {
              return;
            }
